...
**Layout.** Working area: headline, body, a numbered four-step list, a quiet note, one pop-up button that says which Mac the other one is, for the stage's picture of it (**The other Mac's picture**), and a quiet honesty line, which the live line joins when the far end answers. Band 4 is this screen's own footer — `Copy These Steps`, then `Done` as the default, trailing — and the hub's footer and its link row step aside until `Done` (§2.3 band 4, §S1). The stage performs the handoff.
